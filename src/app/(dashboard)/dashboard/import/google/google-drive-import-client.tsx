...
export function GDriveImportClient({ isConnected, oauthError }: { isConnected: boolean; oauthError?: string }) {
  const muiTheme = useTheme()
  const [items, setItems] = useState<DriveItem[]>([])
  const [loading, setLoading] = useState(false)
  const [folderStack, setFolderStack] = useState<BreadcrumbEntry[]>([{ id: 'root', name: 'My Drive' }])
  ...
  const loadItems = useCallback(async (folderId: string) => {
    ...
    setSelected(new Set())
    try {
      const params = folderId === 'root' ? '' : `?folderId=${encodeURIComponent(folderId)}`
      const res = await fetch(`/api/import/google${params}`)
      if (!res.ok) {
        const data = await res.json() as { error?: string }
        throw new Error(data.error ?? 'Failed to load files')
      }
      setItems(await res.json() as DriveItem[])
    } catch {
      setItems([])
    } finally {
      setLoading(false)
  ...
  const toggleAll = () => {
    const selectableIds = items.filter(i => !i.isFolder).map(i => i.id)
  ...
  const handleImport = async () => {
    ...
    setImportError('')
    try {
      const selectedFiles = items.filter(i => selected.has(i.id))
      const res = await fetch('/api/import/google', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ vaultId, files: selectedFiles }),
      })
      if (!res.ok) {
        const data = await res.json() as { error?: string }
  ...
  if (!isConnected) {
    return (
      <Box sx={{ maxWidth: 560 }}>
        <Typography variant="h6" sx={{ fontWeight: 600 }}>Import from Google Drive</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
          Connect your Google Drive to browse and import files directly into your archive.
        </Typography>

        {oauthError && (
          <Alert severity="error" sx={{ mt: 2 }}>
            {oauthError === 'oauth_denied'
              ? 'Google Drive access was denied. Please try again and allow the requested permission.'
        ...
        <Paper elevation={0} sx={{ mt: 3, p: 4, border: '1px solid', borderColor: 'divider', borderRadius: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          <Box sx={{ width: 48, height: 48, borderRadius: '50%', bgcolor: 'rgba(96,165,250,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <CloudUpload size={24} color={muiTheme.palette.primary.main} />
          </Box>
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="body1" sx={{ fontWeight: 500 }}>Connect Google Drive</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
              Read-only access. We will never modify your Drive files.
            </Typography>
          </Box>
          <Button variant="contained" href="/api/auth/google" disableElevation
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Button variant="outlined" size="small" href="/api/auth/google" color="inherit"
          sx={{ textTransform: 'none', '&:hover': { borderColor: 'primary.main', color: 'primary.main' } }}>
          Reconnect Drive
        </Button>
      </Box>

      {activeImports.length > 0 && (
        <Alert severity="info" icon={false} sx={{ bgcolor: 'rgba(37,99,235,0.08)', border: '1px solid', borderColor: 'primary.main', color: 'primary.main' }}>
      ...
      <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, overflow: 'hidden' }}>
        ...
        <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid', borderBottomColor: 'divider', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1 }}>
          <Breadcrumbs
            separator={<ChevronRight size={14} color={muiTheme.palette.text.secondary} />}
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Button
              ...
              disableElevation
              sx={{ bgcolor: '#2563eb', '&:hover': { bgcolor: '#1d4ed8' }, textTransform: 'none', whiteSpace: 'nowrap' }}
            >
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        {item.isFolder
                          ...
                          : <FileText size={15} color={muiTheme.palette.text.secondary} />}
